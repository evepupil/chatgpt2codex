...
| M2        | MCP Transport、本地工作区工具与单用户 OAuth | 未开始 | M0     | [MCP Server 基础协议入口](./模块设计/mcp-server.md)；[Runtime Kernel](./模块设计/runtime-kernel.md)；[MCP Gateway 与本地 OAuth](./模块设计/mcp-gateway-oauth.md) | 固定 HTTPS/Tunnel 入口可完成首次配对，重启后保持授权，并完成受控文件和命令操作 |
